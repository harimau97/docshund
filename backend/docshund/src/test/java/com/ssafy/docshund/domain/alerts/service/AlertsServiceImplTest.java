...
class AlertsServiceImplTest {
    ...
    @BeforeEach
    void setUp() {
        user1 = userTestHelper.saveUser("admin@gmail.com", "10000", "adminUser", Provider.GOOGLE, true, "안녕하세요", "Backend", true);
        user2 = userTestHelper.saveUser("test1@gmail.com", "10001", "testUser1", Provider.GOOGLE, false, "안녕하세요", "Frontend", true);
        user3 = userTestHelper.saveUser("test2@github.com", "10002", "testUser2", Provider.GITHUB, false, "안녕하세요", "Backend", true);

        Document doc1 = documentRepository.save(new Document("Spring", "Spring Boot", "logoImage", "v1",
                0, Position.BACKEND, "apache", "docLink"));
        OriginDocument originDoc1 = originDocumentRepository.save(new OriginDocument(doc1, 2, "<p>", "<p>And Spring boot is very easy and simple.</p>"));
        translatedDoc1 = translatedDocumentRepository.save(new TranslatedDocument(originDoc1, user3, "그리고 스프링 부트는 아주 쉽고 간단합니다.", 0, Status.VISIBLE));

        article1 = articleRepository.save(new Article(user1, doc1, "Spring Boot 개요 공부", "Spring Boot는 경량 애플리케이션 개발을 위한 프레임워크입니다."));
        comment1 = commentRepository.save(new Comment(null, user1, article1, "부모댓글1"));

        InquiryRequestDto inquiryRequest = new InquiryRequestDto("test@test.com", "로그인이 안돼요 ㅠㅠ", String.valueOf(InquiryCategory.MEMBER), "사실 뻥이에요...ㅋㅋ");
    ...
    @Test
    @DisplayName("다른 사용자의 알림을 삭제하려고 할 때 예외 발생")
    @WithMockCustomOAuth2User
    void testDeleteOtherUsersAlert() {
        when(userUtil.getUser()).thenReturn(user2);
        assertThrows(IllegalArgumentException.class, () -> {
            alertsService.deleteAlert(Long.valueOf(alert1.getAlertId()));
        });
    }
